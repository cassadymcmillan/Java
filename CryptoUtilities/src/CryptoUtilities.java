...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        // If m is zero, then the greatest common divisor of n and m is n, and m is clear.
        if (!m.isZero()) {
            // Get the reminder
            NaturalNumber r = n.divide(m);
            reduceToGCD(m, r);
            // Update n and clear m
            n.transferFrom(m);
        }
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        // Get the reminder
        NaturalNumber r = n.divide(new NaturalNumber2(2));
        // Restore n
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        // If power is zero, n is 1
        if (p.isZero()) {
            n.copyFrom(new NaturalNumber2(1));
        } else if (isEven(p)) {
            // Get the reminder
            NaturalNumber r = p.divide(two);
            powerMod(n, p, m);
            NaturalNumber nCopy = new NaturalNumber2(n);
            // Calculate n square
            n.multiply(nCopy);
            // Transfer the reminder to n
            ...
            // Get the reminder
            NaturalNumber r = p.divide(two);
            NaturalNumber firstCopy = new NaturalNumber2(n);
            powerMod(n, p, m);
            NaturalNumber secondCopy = new NaturalNumber2(n);
            // Calculate n square
            n.multiply(secondCopy);
            // Transfer the reminder to n
            n.transferFrom(n.divide(m));
            NaturalNumber nMod = new NaturalNumber2(firstCopy.divide(m));
            nMod.multiply(n);
            // Transfer the reminder to n
            n.transferFrom(nMod.divide(m));
            // Restore p
            p.multiply(two);
            p.add(r);
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wFirstCopy = new NaturalNumber2(w);
        NaturalNumber wSecondCopy = new NaturalNumber2(w);
        NaturalNumber nMinusOne = new NaturalNumber2(n);
        nMinusOne.decrement();
        // Calculate w ^ 2 mod n
        powerMod(wFirstCopy, new NaturalNumber2(2), n);
        // Calculate w ^ (n-1) mod n
        powerMod(wSecondCopy, nMinusOne, n);
        return (wFirstCopy.equals(new NaturalNumber2(1))
                || !wSecondCopy.equals(new NaturalNumber2(1)));
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(3)) > 0) {
            NaturalNumber nCopy = new NaturalNumber2(n);
            // nCopy minus 2 to satisfy 1 < w < n - 1
            nCopy.decrement();
            nCopy.decrement();
            for (int i = 0; i < 35; i++) {
                // Get the random number
                NaturalNumber random = randomNumber(nCopy);
                // The random number must be greater than 1
                while (random.isZero() || random.compareTo(one) == 0) {
                    random = randomNumber(nCopy);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // 2 and 3 are prime numbers
        if (n.compareTo(new NaturalNumber2(3)) > 0) {
            // If the number is even, add 1 to it
            if (isEven(n)) {
                n.increment();
            }
            // While the number is not a prime number, add 2 to it
            while (!isPrime2(n)) {
