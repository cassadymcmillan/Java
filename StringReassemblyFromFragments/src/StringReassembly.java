...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Check whether the added string is a substring of the set
        for (String s : strSet) {
            if (s.indexOf(str) != -1) {
                b = false;
            }
        }
        // If the added string is not a substring of the set
        if (b) {
            // Add this string to the set
            strSet.add(str);
            for (String s : strSet) {
                if (s.equals(str)) {
                    // Add this string to the temporary set
                    temp.add(s);
                } else {
                    // Check whether the set has substrings of this string
                    if (str.indexOf(s) == -1) {
                        temp.add(s);
                    }
                }
            }
            // Transfer this temporary set to the set
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Check whether it is the end of this file
        while (!input.atEOS()) {
            // Get the set of the no pair strings from this file
            addToSetAvoidingSubstrings(s, input.nextLine());
        }
        return s;
    }
